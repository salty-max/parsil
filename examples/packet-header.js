...
import { uint, sequenceOf, succeed } from '../dist/index.js'
...
const packetParser = sequenceOf([
  ...
  if (res[1].value > 5) {
    const bytes = res[1].value * 4 - 20 // IHL*4 minus the base 20-byte header
    const remainingBytes = Array.from({ length: bytes }, () => uint(8))
