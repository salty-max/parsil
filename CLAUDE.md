...
`Parser<T, E>` defaults `E = ParseError`, the structured error shape every primitive emits. Consumers using a custom error type override it explicitly via `errorMap`. Heterogeneous parser arrays (`choice`, `sequenceOf`) are constrained as `Parser<unknown, E>[]` with shared `E` — variance-clean, no `any` widening. `fail<E>(error)` returns `Parser<never, E>` because it can never produce a result.

**No `any` at the StateTransformerFn boundary.** As of #29, `StateTransformerFn<T, E>` takes `(state: ParserState<unknown, E>) => ParserState<T, E>`. Early-exit error forwarding goes through the `forward<E>(state)` helper, which casts the state's `result` slot from `unknown` to `never` in one place — `never` unifies with any successful return type so the caller doesn't need an extra annotation. If you find yourself reaching for `any` to silence a typecheck inside a parser primitive, stop: either the parser is doing something the model didn't anticipate (worth a design discussion), or `forward(state)` is what you want.
